...
interface Monument {
  id: string;
  title: string;
  category: 'huaca' | 'museo' | 'sitio_historico';
  district: string;
...
export function MonumentsManager() {
  ...
  const handleStatusChange = (id: string, newStatus: Monument['status']) => {
    setMonuments(prev => 
      prev.map(monument => 
        monument.id === id 
...
function MonumentForm({ onClose }: { onClose: () => void }) {
  return (
