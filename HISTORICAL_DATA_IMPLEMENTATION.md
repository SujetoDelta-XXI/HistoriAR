...
### Fase 3: Endpoint para App Móvil (Opcional)
```javascript
GET /api/mobile/monuments/:id/complete
Response: {
  monument: { /* datos básicos */ },
  historicalData: [
    { id, title, description, imageUrl, order },
    ...
  ],
  model3D: { /* URL del modelo activo */ }
}
```

### Mejoras Futuras (Opcional)
1. Galería de imágenes múltiples por entrada (usar campo `oldImages`)
2. Editor de texto enriquecido para descripciones
3. Categorización de entradas (descubrimiento, cultura, arquitectura, etc.)
4. Versionado de entradas históricas
5. Exportación de información a PDF
6. Búsqueda de texto completo en descripciones
...
### Frontend
1. Navegar a "Información Monumentos"
2. Seleccionar un monumento
3. Crear una entrada con imagen
4. Editar la entrada
5. Reordenar entradas
6. Eliminar entrada
7. Verificar que la imagen se eliminó de GCS

## Notas Técnicas

- Las imágenes se almacenan en `images/monuments/{monumentId}/`
- Cada entrada tiene su propia imagen (campo `imageUrl`)
- El campo `order` permite ordenar las entradas
- El campo `createdBy` permite auditoría
- Las imágenes antiguas se pueden mantener en `oldImages` para galerías futuras
- La eliminación de entradas también elimina las imágenes de GCS
